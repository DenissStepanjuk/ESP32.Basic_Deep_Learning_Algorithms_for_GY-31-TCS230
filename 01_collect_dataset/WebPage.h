...
"      <div>Color from Sensor: </div>"
      /** Выпадающий список (select) цветов для детекции. **/
"      <div id='container'></div>"

      /** Кнопки:
        - DETECT - Захватить параметры цвета с датчика и добавить в датасет.
        - DOWNLOAD - Скачать датасет.**/
"      <p><button type='button' id='BTN_Detect'> DETECT </button> <button type='button' id='BTN_Download'> DOWNLOAD </button></p>"
      /** Кнопки:
        - Окно для ввода индекса удаляемой строки.
        - DELETE - отвечающая за удаление строки.
        - REFRESH PAGE - перезагрузить страничку **/
"      <input id='idx_delete' type='text' maxlength='14'>"
"      <p><button type='button' id='BTN_Delete'> DELETE </button> <button onclick='location.reload();'>REFRESH PAGE</button></p>"
...
"      var btn_del = {type: 'delete', value: inputValue};"
...
"        var msg = {type: 'selected_color', value: selectedValue};"
